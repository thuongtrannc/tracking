...
class Vis():
    def __init__(self, data_path='', clone_path=''):
    ...
    def plot_boxes(self, objects):
        ...
        for i in range(2):
            ...
            for j in range(boxes.shape[0]):
                corners = self.get_corners(boxes[j])
                k = cnt * 4
                cnt += 1
                con = [ [k, k + 1],
                        [k + 1, k + 2],
        ...
        num_points = (objects[0].shape[0] + objects[1].shape[0]) * 4
    ...
    def load_data(self):
        ...
        self.clone_data = np.loadtxt(self.clone_path, delimiter=',')
    ...
    def update_frame(self, event):
        if self.draw_boxes == False:

            objects_gt = self.data
            objects_clone = self.clone_data
            self.plot_boxes([objects_gt, objects_clone])
            self.draw_boxes = True


if __name__ == '__main__':
    vis = Vis('tracking/data/test_hungarian.txt', 'tracking/data/test_hungarian_clone.txt')
    vis.run()
