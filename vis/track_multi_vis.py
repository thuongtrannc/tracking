...
class Vis():
    ...
    def plot_boxes(self, objects, type = 'gt'):
        ...
        # Estimation
        self.mot.update(boxes, self.timer_estimate)
        self.timer_estimate += self.dt

        save_track = []
        for object in self.mot.tracks:
            new_object = dict(object)
            del new_object['filter']
            save_track.append(new_object)

        for i in range(len(save_track)):
            box = [save_track[i]['x'], save_track[i]['y'], save_track[i]['yaw'], save_track[i]['w'], save_track[i]['l']]
